...
- **Scale:** Spark and Flink are proven on thousands of machines, with shuffles, spilling and
  ...
  - Since round 11, Pondra has shuffles, spilling, partitions, and table metadata that stays
    ...
    answering exactly as on one node, the same every run. Round 15 split big tables that share a
...
| Batch `$changelog` / `$binlog` reads | `/watch?after=` replays every change, deletes included; `&changes=true` gives each row's `_change_type` (insert, update_preimage, update_postimage, delete) and `_row_id`, as Delta's change data feed; MCP `changes` |
| UPDATE / DELETE without the full primary key (for GDPR erasure) | UPDATE / DELETE / MERGE with any WHERE, on append tables too (round 19); a purge rewrites the files without the old rows, so they are gone from Delta and Iceberg as well |
...
|---|---|---|---|
| **Multi-machine evidence** | Everything above is one box | Round 18: TPC-H SF10 on 3 GitHub runners joined over the public internet (17–54 ms, 51–150 MB/s): every answer equal to one node's, but 46.6 s against 23.1 s, the shuffles paying about 15 ms per MB moved. Round 19: a query spreads only when what its plan would move, at the measured speed of the slowest link, costs less than the work it shares out (`guard.rs`), so on such a network a cluster runs what doesn't pay on one node. Next: the bench again with the guard on, then 3–20 VMs in one data centre against S3/R2 | Near-linear ingest and query scaling, failover times |
...
| **Schema evolution** | Tables change; Fluss 1.0 lists it as a gap too | Round 10: `ALTER TABLE … ADD COLUMN` (old rows read null; Delta and Iceberg follow). Round 19: `ALTER TABLE … SET (publish, cluster_by, ttl)`. Next (round 20): column ids in the files, then renames, drops, defaults, type widening | ✓ adding a column under load (`harness.py alter`) |
| **A database's SQL** | What Snowflake, Databricks SQL and Postgres users type first: schemas, three-part names, views, `DROP`, `CREATE TABLE … AS` | Round 18: a lake is a database of schemas (`lake.schema.table`; attached lakes are databases too), `CREATE`/`DROP SCHEMA`, `DROP TABLE`, CTAS, stored views and `CREATE MATERIALIZED VIEW` (the streaming view), listed over Postgres, Flight SQL, Iceberg REST and MCP. Round 19: `UPDATE`/`DELETE`/`MERGE` on every table, from one snapshot in one commit, with system columns (`_row_id` kept through changes as Iceberg v3's row lineage keeps it, `_version`, `_created_at`, `_updated_at`); views that add up or are row by row take changed rows back; `CREATE DATABASE`, `CHECKPOINT`. Next: materialized views filled from existing rows; renames | ✓ `harness.py schemas`, `harness.py changes` |
