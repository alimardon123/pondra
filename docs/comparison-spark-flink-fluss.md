# Pondra vs Spark, Flink and Fluss: batch, streaming and latency (round 4)

**Date:** 2026-09-20 · **Machine:** one 2-vCPU, 7 GB sandbox VM, local disk; every engine ran alone, one at a time
...
Spark and Flink numbers are from round 3 (same machine, same scripts); Pondra's were re-measured on the round-4 binary.

## Footprint

| | Pondra | Spark 4.2 | Flink 2.3 |
|---|---|---|---|
| What you install | one binary, **89.2 MB** (30 MB gzip, 16.8 MB xz), no runtime | 485 MB PySpark + a 286 MB JVM | 353 MB PyFlink + a 286 MB JVM |
| Start to first query | **0.03 s** | 4.1 s | 5.2–5.4 s |
| Idle memory | **37 MB** | — | — |
| Peak memory in these runs | **258–639 MB** (2.0 GB saturated for 60 s) | 0.7–1.5 GB | 1.2–3.1 GB |
