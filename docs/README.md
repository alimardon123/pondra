...
| File | What it is |
|---|---|
| `prototype-status.md` | Where the prototype stands: what it does, every measured number, what's missing (round 16, the current one) |
...
| `adr-017-streams-on-their-own-time.md` | Current design: the watermark taken from the stream's own event time (windows closed when the data is past them), session windows emitted once, whole, and point-in-time joins (`ASOF JOIN`) ad hoc, across the nodes and in views over a stream — against DuckDB's answers |
...
`../logs/round4/` to `../logs/round15/` hold the raw output of the test runs these reports quote.
