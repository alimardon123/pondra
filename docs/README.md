...
| File | What it is |
|---|---|
| `prototype-status.md` | Where the prototype stands: what it does, every measured number, what's missing (round 10, the current one) |
...
| `adr-011-open-doors.md` | Current design: the Kafka protocol (producers, Debezium, consumers, groups), the Iceberg REST catalog, `ALTER TABLE … ADD COLUMN`, event-time windows that emit once, JSON functions, timestamps in µs |
...
`../logs/round4/` to `../logs/round10/` hold the raw output of the test runs these reports quote.
