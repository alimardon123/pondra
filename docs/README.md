...
| File | What it is |
|---|---|
| `prototype-status.md` | Where the prototype stands: what it does, every measured number, what's missing (round 9, the current one) |
...
| `adr-010-anywhere.md` | Current design: the bucket inbox (writes from any network), attached lakes (several leaders on one bucket), SQL writes, the Postgres protocol, the Python client, MCP for AI agents, vector search, tokens, size-tiered keyed compaction, TTL, the change feed |
...
`../logs/round4/` to `../logs/round9/` hold the raw output of the test runs these reports quote.
