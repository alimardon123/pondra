...
| File | What it is |
|---|---|
| `prototype-status.md` | Where the prototype stands: what it does, every measured number, what's missing (round 18, the current one) |
| `roadmap.md` | What's left and in what order after round 18: six tracks (runs anywhere, in-process and in the browser, proof at scale, correctness, ready for a team, depth), lessons from DuckDB-WASM and PGlite, rounds 19–25, and the decisions only the owner can make |
...
| `adr-019-a-database-you-can-shape.md` | Current design: a lake is a database of schemas (`lake.schema.table`; other lakes attached in SQL, `ATTACH … AS …`, for queries across databases), DDL in SQL (`CREATE`/`DROP SCHEMA`, `DROP TABLE`, CTAS), stored and materialized views and queries over views across the nodes, every door listing the schemas, a follower that waits for a new lake's catalog, memory on Windows and macOS, and the cluster bench measuring the network |
| `adr-018-install-anywhere.md` | Round-17 design: a Linux binary for glibc 2.17, `pip install pondra` and `npm install pondra` with `local()`, a SQL shell, nodes that stop when whoever started them does, small-machine and proxy defaults, `sum` over DOUBLE the same in any order, a lite build measured and rejected, and a release workflow for five platforms |
...
`../logs/round4/` to `../logs/round18/` hold the raw output of the test runs these reports quote.
